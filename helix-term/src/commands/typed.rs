...
fn complete_command_args(
    ...
    use command_line::{CompletionState, ExpansionKind, Token, TokenKind, Tokenizer};
    ...
    match token.kind {
        ...
        TokenKind::Expand | TokenKind::Expansion(ExpansionKind::Shell) => {
            complete_expand(token.content.as_ref(), offset + token.content_start)
        }
...
fn complete_expand(content: &str, offset: usize) -> Vec<ui::prompt::Completion> {
    use command_line::{ExpansionKind, TokenKind, Tokenizer};

    let mut start = 0;

    while let Some(idx) = content[start..].find('%') {
        let idx = start + idx;
        if content.as_bytes().get(idx + '%'.len_utf8()).copied() == Some(b'%') {
            // Two percents together are skipped.
            start = idx + ('%'.len_utf8() * 2);
        } else {
            let mut tokenizer = Tokenizer::new(&content[idx..], false);
            let token = tokenizer
                .parse_percent_token()
            ...
            match token.kind {
                ...
                TokenKind::Expansion(ExpansionKind::Shell) => {
                    return complete_expand(&token.content, local_offset)
                }
                TokenKind::ExpansionKind => {
